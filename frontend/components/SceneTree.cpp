...
void SceneTree::resizeEvent(QResizeEvent *event)
{
	...
	if (gridMode) {
		if (!count) {
			...
			int w = rect.width() - 1; // need to subtract 1 for some reason
...
void SceneTree::dropEvent(QDropEvent *event)
{
	...
	if (gridMode) {
		...
		QRect rect = contentsRect();
		QPointF position(rect.x() + column * itemWidth, rect.y() + row * itemHeight);
		event = new QDropEvent(position, event->possibleActions(), event->mimeData(), event->buttons(),
				       event->modifiers());
...
void SceneTree::RepositionGrid(QDragMoveEvent *event)
{
	...
	for (int i = 0; i < count; i++) {
		auto *wItem = item(i);
		QPoint position = QPoint((i % columns) * itemWidth, (i / columns) * itemHeight);
		QModelIndex index = indexFromItem(wItem);
		setPositionForIndex(position, index);
	}
}
