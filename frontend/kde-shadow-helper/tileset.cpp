...
//______________________________________________________________
TileSet::TileSet(const QPixmap &source, int w1, int h1, int w2, int h2) : _w1(w1), _h1(h1), _w3(0), _h3(0)
{
	_pixmaps.reserve(9);
	if (source.isNull()) {
		return;
...
void TileSet::render(const QRect &constRect, QPainter *painter, Tiles tiles) const
{
	...
	if (bits(tiles, Top | Left)) {
		painter->drawPixmap(x0, y0, _pixmaps.at(0), 0, 0, wLeft * devicePixelRatio(_pixmaps.at(0)),
				    hTop * devicePixelRatio(_pixmaps.at(0)));
	}
	if (bits(tiles, Top | Right)) {
		painter->drawPixmap(x2, y0, _pixmaps.at(2), (_w3 - wRight) * devicePixelRatio(_pixmaps.at(2)), 0,
				    wRight * devicePixelRatio(_pixmaps.at(2)), hTop * devicePixelRatio(_pixmaps.at(2)));
	}
	if (bits(tiles, Bottom | Left)) {
		painter->drawPixmap(x0, y2, _pixmaps.at(6), 0, (_h3 - hBottom) * devicePixelRatio(_pixmaps.at(6)),
				    wLeft * devicePixelRatio(_pixmaps.at(6)),
				    hBottom * devicePixelRatio(_pixmaps.at(6)));
	}
	if (bits(tiles, Bottom | Right)) {
		painter->drawPixmap(x2, y2, _pixmaps.at(8), (_w3 - wRight) * devicePixelRatio(_pixmaps.at(8)),
