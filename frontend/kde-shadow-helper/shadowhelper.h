...
struct ShadowParams {
	...
	ShadowParams(const QPoint &offset, int radius, qreal opacity) : offset(offset), radius(radius), opacity(opacity)
	{
	}
...
public:
	...
	//* frame radius
	static constexpr qreal frameRadius(const int penWidth = PenWidth::NoPen, const qreal bias = 0);

	static ShadowHelper *sharedInstance();

	//* constructor
	explicit ShadowHelper();
