...
namespace {
using ShadowHelper::CompositeShadowParams;
using ShadowHelper::ShadowParams;
...
bool ShadowHelper::isX11()
{
	static const bool s_isX11 = KWindowSystem::isPlatformX11();
	return s_isX11;
}

constexpr qreal ShadowHelper::frameRadius(const int penWidth, const qreal bias)
{
	return qMax(Metrics::Frame_FrameRadius - (0.5 * penWidth) + bias, 0.0);
}

ShadowHelper *ShadowHelper::sharedInstance()
{
	static ShadowHelper *shadowHelper = new ShadowHelper();
	return shadowHelper;
}

//_____________________________________________________
ShadowHelper::ShadowHelper() : QObject() {}
...
TileSet ShadowHelper::shadowTiles(QWidget *widget)
{
	...
	const qreal frameRadius = ShadowHelper::frameRadius();
	...
	const QMargins margins =
		QMargins(boxRect.left() - outerRect.left() - Metrics::Shadow_Overlap - params.offset.x(),
	...
	_shadowTiles =
		TileSet(QPixmap::fromImage(std::move(shadowTexture)), innerRectTopLeft.x(), innerRectTopLeft.y(), 1, 1);
...
const QVector<KWindowShadowTile::Ptr> &ShadowHelper::createShadowTiles()
{
	...
	if (_tiles.isEmpty()) {
		_tiles = {createTile(_shadowTiles.pixmap(1)), createTile(_shadowTiles.pixmap(2)),
			  createTile(_shadowTiles.pixmap(5)), createTile(_shadowTiles.pixmap(8)),
			  createTile(_shadowTiles.pixmap(7)), createTile(_shadowTiles.pixmap(6)),
			  createTile(_shadowTiles.pixmap(3)), createTile(_shadowTiles.pixmap(0))};
	}
...
QMargins ShadowHelper::shadowMargins(QWidget *widget) const
{
	...
	const QSizeF shadowSize = BoxShadowRenderer::calculateMinimumShadowTextureSize(boxSize, params.shadow1.radius,
...
qreal ShadowHelper::devicePixelRatio(QWidget *widget)
{
	...
	static const bool s_isWayland = qgetenv("XDG_SESSION_TYPE") == "wayland";
	return s_isWayland ? 1 : widget->devicePixelRatioF();
}
