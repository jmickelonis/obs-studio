...
ffz.setAttribute('src','https://cdn.frankerfacez.com/script/avalon.js');\
 ...
 */
BrowserDock *TwitchAuth::addDock(const std::string &name, const std::string &localeName, const std::string &title,
				 const std::string &url, const std::string &startupScript,
				 const DockOptions &dockOptions)
{
	...
	BrowserDock *dock = new BrowserDock(windowTitle);
...
void TwitchAuth::LoadSecondaryUIPanes()
{
	...
	DockConfig docks[] = {
		{"twitchActiveMods",
		 "Twitch.ActiveMods",
		 "Active Mods",
		 "https://dashboard.twitch.tv/popout/u/" + name + "/stream-manager/active-mods?uuid=" + uuid,
		 {}},
		{"twitchFeed",
		 "Twitch.ActivityFeed",
		 "Activity Feed",
		 "https://dashboard.twitch.tv/popout/u/" + name + "/stream-manager/activity-feed?uuid=" + uuid,
		 {}},
		{"twitchAdManager",
		 "Twitch.AdManager",
		 "Ad Manager",
		 "https://dashboard.twitch.tv/popout/u/" + name +
			 "/stream-manager/streamer-ads-manager-panel?uuid=" + uuid,
		 {}},
		{"twitchAutoModQueue",
		 "Twitch.AutoModQueue",
		 "AutoMod Queue",
		 "https://dashboard.twitch.tv/popout/u/" + name + "/stream-manager/auto-mod-queue?uuid=" + uuid,
		 {}},
		{"twitchModActions",
		 "Twitch.ModActions",
		 "Mod Actions",
		...
		{"twitchPredictions",
		 "Twitch.Predictions",
		 "Predictions",
		 "https://dashboard.twitch.tv/popout/u/" + name + "/stream-manager/predictions?uuid=" + uuid,
		 {}},
		{"twitchRewardQueue",
		 "Twitch.RewardQueue",
		 "Reward Queue",
		 "https://dashboard.twitch.tv/popout/u/" + name + "/stream-manager/reward-queue?uuid=" + uuid,
		 {}},
		{"twitchStats",
		 "Twitch.Stats",
		 "Stats",
		 "https://www.twitch.tv/popout/" + name + "/dashboard/live/stats",
		 {200, 250},
		 false,
		 true},
		{"twitchStreamHealth",
		 "Twitch.StreamHealth",
		 "Stream Health",
		 "https://dashboard.twitch.tv/popout/u/" + name + "/stream-manager/stream-health?uuid=" + uuid,
		 {}},
		{"twitchInfo",
		 "Twitch.StreamInfo",
		 "Stream Info",
		 "https://dashboard.twitch.tv/popout/u/" + name + "/stream-manager/edit-stream-info",
		 {},
		 true},
		{"twitchQuickActions",
		 "Twitch.QuickActions",
		 "Quick Actions",
		 "https://dashboard.twitch.tv/popout/u/" + name + "/stream-manager/quick-actions?uuid=" + uuid,
		 {}},
		{"twitchUnbanRequests",
		 "Twitch.UnbanRequests",
		 "Unban Requests",
		 "https://dashboard.twitch.tv/popout/u/" + name + "/stream-manager/unban-requests?uuid=" + uuid,
		 {}},
		{"twitchWhispers",
		 "Twitch.Whispers",
		 "Whispers",
	...
	for (DockConfig config : docks) {
		BrowserDock *dock =
			addDock(config.name, config.localeName, config.title, config.url, script, config.dockOptions);
		dockMap[dock->objectName()] = dock;
		if (config.center) {
			// Center the dock if specified...
			QSize frameSize = dock->frameSize();
			dock->move(pos.x() + size.width() / 2 - frameSize.width() / 2,
