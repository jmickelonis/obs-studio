...
void TitleBarButton::paintEvent(QPaintEvent *)
{
	...
	if (style->styleHint(QStyle::SH_DockWidget_ButtonsHaveFrame, nullptr,
			     stockButton)) {
...
QSize TitleBarLayout::sizeHint() const
{
	...
	bool vertical =
		dock->hasFeature(QDockWidget::DockWidgetVerticalTitleBar);
	...
	int w = (vertical ? closeSize.height() : closeSize.width()) +
		(vertical ? floatSize.height() : floatSize.width()) + h +
		3 * margin;
...
void TitleBarLayout::setGeometry(const QRect &)
{
	...
	QRect rect = style->subElementRect(QStyle::SE_DockWidgetCloseButton,
					   &opt, dock);
	if (!rect.isNull())
		item->setGeometry(rect);

	item = items[FloatButton];
	rect = style->subElementRect(QStyle::SE_DockWidgetFloatButton, &opt,
				     dock);
	if (!rect.isNull())
		item->setGeometry(rect);
}
...
TitleBarWidget::TitleBarWidget(OBSDock *dock) : QWidget(dock)
{
	...
	closeButton = new TitleBarButton(dock->findChild<QAbstractButton *>(
		"qt_dockwidget_closebutton"));
	floatButton = new TitleBarButton(dock->findChild<QAbstractButton *>(
	...
	connect(closeButton, &QAbstractButton::clicked, this,
		&TitleBarWidget::onCloseClicked);
	connect(floatButton, &QAbstractButton::clicked, this,
		&TitleBarWidget::onFloatClicked);
	connect(dock, &QDockWidget::featuresChanged, this,
		&TitleBarWidget::onFeaturesChanged);
	connect(dock, &QDockWidget::topLevelChanged, this,
		&TitleBarWidget::onTopLevelChanged);
}
...
bool TitleBarWidget::event(QEvent *event)
{
	...
	case QEvent::ContextMenu:
		...
		if (QMenu *menuDocks =
			    App()->GetMainWindow()->findChild<QMenu *>(
				    "menuDocks")) {
...
void TitleBarWidget::onTopLevelChanged(bool)
{
	...
	OBSDock *dock = getDock();
	if (dock->isFloating() &&
	    dock->mouseState == OBSDock::MouseState::NotPressed)
		dock->setDropShadow(true);
#endif
}

OBSDock::OBSDock(const QString &title, QWidget *parent)
...
#ifdef _WIN32
bool OBSDock::nativeEvent(const QByteArray &eventType, void *message,
			  long *result)
{
	if (!isFloating())
		goto END;
	...
	case WM_SIZING:
		...
		// allowing us to prevent illegal resize operations
		{
			...
			if (edge == WMSZ_TOPLEFT || edge == WMSZ_TOP ||
			    edge == WMSZ_TOPRIGHT) {
				if (rect.top < yMin) {
...
bool OBSDock::onMouseMoved(QMouseEvent *event)
{
	if (mouseState == MouseState::CtrlPressed) {
		qreal dragDistance =
			(event->pos() - pressPosition).manhattanLength();
